...
int main(void)
{
      ...
      //delay(100);
            ...
            i2c_transfer(HMC_ADDRESS, cmd_array, data_array, 1, 6, I2C_FLAG_WRITE_READ);
            										  ...
            										  //Refer Pg 11 from HMC5833L
			XVal = (data_array[1]<<8) | data_array[0];//As Data Out XA(0x03) - MSB
													  //As Data Out XB(0x04) - LSB

			ZVal = (data_array[3]<<8) | data_array[2];//As Data Out ZA(0x05) - MSB
			  	  	  	  	  	  	  	  	  	  	  //As Data Out ZB(0x06) - LSB

			YVal = (data_array[5]<<8) | data_array[4];//As Data Out YA(0x05) - MSB
  	  	  	  	  	  	  	  	  	  	  	  	  	  //As Data Out YB(0x06) - LSB

            // Infinite loop
    	while (1){


    					// Measurement data starts at DATAX0, and ends at DATAZ1, 6 bytes long,Reinnitialized
    		           	cmd_array[0] = DataRegisterBegin;
